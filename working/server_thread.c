...
#define NUM_VEHICLES                    256
#define VEHICLEID 1
#define PI	  3.14

void *process_measurements(void* argv);

int clientsock[NUM_VEHICLES];
int rval;
...
int main(int argc, char *argv[]) {
  ...
  int i,c;
  uint16_t plen;
  pthread_t client_threads[NUM_VEHICLES];
  int index = 0;
  uint8_t* value;
  int k;
  uint16_t bl_len;
  ...
  while((clientsock = accept(sock, (struct sockaddr *)&client,(socklen_t*)&c))) {  
    ...
    if(clientsock == -1)
      ...
      new_sock = malloc(1);
      *new_sock = clientsock;
      if(index < NUM_VEHICLES) {
	pthread_create(&client_threads[index],NULL,process_measurements,(void*)new_sock);
      }
  ...
  while(1) {
	...
	if ( rval < 0 ) {
	  ...
	  /* pdu is the data packet which contains following: 
	     ---> Four Bytes of Wheel Size
	     ---> One Byte of Vehicle ID
	     ---> Two bytes as Length of the packet.
	     ---> One byte of OPCODE
	     ---> Two Bytes of Handle
	     ---> The remaining bytes are data basing on the handle.(Flags + Raw Data)
	  ...
	  */ /*
	  ...
	  case SPEED_HANDLE:
	    /*
	       ...
	       --> Total Distance is still cumulative. We can use a global variable to store prev_total_dist travelled. 
	    */
/*
	    flags = pdu[10];
		...
		printf ("\n\t\tDistance travelled during elapsed time :%.2fm \n", distance);
...
*/
void *process_measurements(void* vargv) {
  int *myid = (int*)vargv;
  ...
  while(1) {
    memset(pdu, 0, sizeof(pdu));
    rval = recv(*myid, pdu, sizeof(pdu), 0);
    if(rval < 0) {
      perror("Protocol Error...Receiving Failed.");
    }
    else{
	...
	*/
	...
	break;
      ...
      default:
	printf("The data we received is from a different handle. We don't need this data.\n");
	break;
      }
      battery_level = 100;
      generate_html_page ();
